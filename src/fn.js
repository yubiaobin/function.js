...
(function(root,factory){
	...
	* @return {[type]}
	*/
	var complement = function(arr1, arr2) {
		// 声明结果数组和临时数组
		var   result = [], 
	...
	* @return {[type]}
	*/
	var getComplementArray = function(arr1, arr2) {
		//重置数组
	...
	* @return {[type]}     [description]
	*/
	var getDivisorsArray = function(num){
	    var arr=[];
	...
	* @return {[type]}
	*/
	var charUpper = function(text, delimiter){
		var str = "";
	...
	* @return {[type]}     [description]
	*/
	var removeRepeatCharacter = function(str){
		var arr = [],
			final_arr = [];
		arr = str.split('')
		for(var i = 0,len = arr.length; i < len; i++){
	...
	* @return {[type]}
	*/
	var cssRemoveDelimiter = function(style){
		var doStyel;
		if(style.indexOf("-") !== -1){
